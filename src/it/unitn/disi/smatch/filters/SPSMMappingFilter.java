...
//		filteredMapppings = new MappingElement[sourceSize];
    ...
    public IContextMapping<INode> filter(IContextMapping<INode> mapping) throws MappingFilterException {
        ...
        if (numberofOriginalMappings > 0) {
            ...
            log.info("No links left after applying the SPSM mapping filter. " +
     ...
     * @param semantic_relation Defined relation SEMANTIC_RELATION
...
//			spsmMapppings[row] = cNodeMatrix[row][col];
    ...
    private void resolveStrongestMappingConflicts(int sourceNodeIndex,
        ...
        log.info("more than one strongest relation for " +
...
//		spsmMapppings[sourceNodeIndex] = strongest.get(strongestIndex);
    ...
    /**
     * Checks if source and target are structure preserving:
     * function to function and argument to argument match.
     *
     * @param source
     * @param target
    ...
    private boolean morePrecedent(IMappingElement<INode> source, IMappingElement<INode> target) {
        ...
        //TODO Juan, remove unused code, at least after all refactoring is done.
//		if(source != null && target == null){
//			return true;
//		}
...
//	}
     ...
     * IDK = 5
     *
     * @param relation relation SEMANTIC_RELATION

...
/**
 * Performs all the operations related to linguistic preprocessing. Italian variant.
 ...
 */
public class ContextPreprocessorIt extends Configurable implements IContextPreprocessor {

    private static final Logger log = Logger.getLogger(ContextPreprocessorIt.class);
    ...
    public void processNode(INode node) throws ContextPreprocessorException {
        try {
            ...
            if ((0 == wnSense.size()) && !(("top".equals(labelOfNode) || "thing".equals(labelOfNode)) && !node.hasParent()) && (!meaninglessWords.contains(labelOfNode + " ")) && (isTokenMeaningful(labelOfNode))) {
